...
function getOpenAIClient(): OpenAI {
  if (!openai) {
    ...
    if (!apiKey || apiKey === 'your-api-key-here') {
      throw new Error('OpenAI API key not configured. Please add OPENAI_API_KEY to .env.local file');
    }
...
async function parseHypothesisWithAI(hypothesisText: string): Promise<ParsedHypothesis> {
...
async function generateKnowledgeCardWithAI(
  intervention: string,
  outcome: string,
  category: string
...
export async function POST(request: NextRequest) {
  try {
    ...
    switch (action) {
      case 'parse': {
        const { hypothesisText } = params;
        if (!hypothesisText || typeof hypothesisText !== 'string') {
          return NextResponse.json(
            { error: 'hypothesisText is required and must be a string' },
            { status: 400 }
          );
        }
        result = await parseHypothesisWithAI(hypothesisText);
        break;
      }

      case 'knowledgeCard': {
        const { intervention, outcome, category } = params;
        if (!intervention || !outcome || !category) {
          return NextResponse.json(
            { error: 'intervention, outcome, and category are required' },
            { status: 400 }
          );
        }
        result = await generateKnowledgeCardWithAI(intervention, outcome, category);
        break;
      }

      case 'chat': {
        const { messages, extractInfo } = params;
        ...
        result = await chatWithAI(messages, extractInfo === true);
        break;
      }
